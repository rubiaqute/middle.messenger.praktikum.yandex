...
class ChatPage extends Block<BasicBlockProps> {
    ...
    changeActiveChat(chatId: number) {
        const newActiveChat = chatList.find((chatItem) => chatItem.id === chatId) ?? chatList[0];

        (this.childrenNodes.ChatBody as unknown as ChatBody).updateChatBody(newActiveChat)
    }
